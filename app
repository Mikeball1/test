...
scroll("However scroll losing a challenge by a margin of less than 5 will result in the decrease of a stat based on the type of challenge\n")
...
check()
print("--------------------------------------------")
scroll('''You are entering the second fight. This is a test of Strength. Depending on the outcome of this game, you may lose STR or gain STR''')
ffight()
winorlose()
if countere>counter:#if the enemy has rolled a total greater than the user, it will enter this if statement, otherwise it will enter the else statement
